	...
	public function indexPoliGigi(Request $request)
	{
	    ...
	    if ($request->ajax()) {
	        ...
	        return DataTables::of($actionsQuery)
	            ...
	            ->addColumn('action', function ($row) use ($rs, $dokter, $routeName, $obats, $poli, $diagnosa) {
	                $editModal = view('component.modal-edit-action', [
	                    'rs' => $rs,
	                    'action' => $row,
	                    'routeName' => $routeName,
	                    'dokter' => $dokter,
	    ...
	    return view('content.action.index', compact(
